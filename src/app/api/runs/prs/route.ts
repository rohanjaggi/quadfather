...
export async function GET(request: NextRequest) {
  try {
    ...
    if (fastestPace?.pace_per_km) {
      const mins = Math.floor(fastestPace.pace_per_km)
      const secs = Math.round((fastestPace.pace_per_km - mins) * 60)
      prs.push({
        label: 'Fastest Pace',
    ...
    if (fiveKRuns.length > 0) {
      ...
      const mins = Math.floor(fastest5k.duration_seconds / 60)
      const secs = fastest5k.duration_seconds % 60
      prs.push({
        label: 'Fastest 5K',
        value: `${mins}:${secs.toString().padStart(2, '0')}`,
    ...
    if (tenKRuns.length > 0) {
      ...
      const mins = Math.floor(fastest10k.duration_seconds / 60)
      const secs = fastest10k.duration_seconds % 60
      prs.push({
        label: 'Fastest 10K',
        value: `${mins}:${secs.toString().padStart(2, '0')}`,
